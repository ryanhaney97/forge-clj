(ns forge-clj.world
  ...
  (:import
   ...
   [net.minecraft.world World WorldType]
   [net.minecraft.util MathHelper BlockPos]
...
(defn clj-generate
  ...
  (let [dimension-id (.getDimensionId (.-provider world))
...
(defn run-default-generator
  ...
  (loop [chance chances]
    (when (< 0 chance)
      (.generate generator world rand-obj (BlockPos. (int (+ (* 16 chunk-x) (.nextInt rand-obj 16))) (int (+ height1 (.nextInt rand-obj (abs (- height1 height2))))) (int (+ (* 16 chunk-z) (.nextInt rand-obj 16)))))
      (recur (dec chance)))))
